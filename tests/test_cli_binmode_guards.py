...
class TestCliBinmodeGuards(unittest.TestCase):
    ...
    def test_stamp_alias_rejects_overlapping_input_and_out(self) -> None:
    ...
    def test_stamp_alias_json_rejects_overlapping_input_and_out_with_failure_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            input_dir = tmp_path / "input"
            out_dir = input_dir / "out"
    ...
    def test_stamp_alias_rejects_input_nested_under_out(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
    ...
    def test_stamp_alias_json_rejects_input_nested_under_out_with_failure_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            out_dir = tmp_path / "out"
            input_dir = out_dir / "input"
    ...
    def test_stamp_alias_rejects_json_with_print_seed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            input_dir = tmp_path / "input"
            out_dir = tmp_path / "out"
    ...
    def test_stamp_alias_seed_flag_misuse_leaves_output_tree_uncreated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            input_dir = tmp_path / "input"
            out_dir = tmp_path / "out"
    ...
    def test_consume_bin_rejects_overlapping_source_bin_and_out(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
    ...
    def test_stamp_bin_alias_consume_bin_json_rejects_overlapping_source_bin_and_out_with_failure_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            source_bin = tmp_path / "source_bin"
            out_dir = source_bin / "out"
    ...
    def test_stamp_bin_alias_consume_bin_json_low_watermark_failure_emits_failure_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            source_bin = tmp_path / "source_bin"
            out_dir = tmp_path / "out"
