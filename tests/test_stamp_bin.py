...
class TestConsumeBin(unittest.TestCase):
    ...
    def test_consume_from_source_bin_preserves_stage_dir_when_recovery_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            source_bin = tmp_path / "source_bin"
            out_dir = tmp_path / "out"
    ...
    def test_consume_from_source_bin_keeps_unrecovered_files_in_failed_stage_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            source_bin = tmp_path / "source_bin"
            out_dir = tmp_path / "out"
