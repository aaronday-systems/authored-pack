...
class TestTuiAuditQuickWins(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.m = load_tui_module("authored_pack_tui_audit_quick_wins")
    ...
    def test_prompt_str_curses_shortens_folder_defaults_on_narrow_terminals(self) -> None:
        m = self.m
        stdscr = RecordingStdScr(inputs=[10])
        long_default = "/Users/example/Desktop/Screenshot 2026-03-26 at 17.04.28.png"
    ...
    def test_noisy_failed_assemble_renders_with_failure_palette(self) -> None:
        m = self.m
        state = self._state()
        state.insane = True
    ...
    def test_assemble_requires_explicit_input_choice_before_running(self) -> None:
        m = self.m
        state = self._state()
    ...
    def test_assemble_preview_ignores_stale_failure_logs_when_not_in_result_state(self) -> None:
        m = self.m
        state = self._state()
        state.selected = state.menu.index("Assemble")
    ...
    def test_assemble_review_preview_stays_readable_on_narrow_terminal(self) -> None:
        m = self.m
        state = self._state()
        state.selected = state.menu.index("Assemble")
    ...
    def test_run_assemble_and_verify_from_config_round_trips_real_pack(self) -> None:
        m = self.m
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
