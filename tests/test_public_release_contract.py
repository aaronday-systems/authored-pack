...
class TestPublicReleaseContract(unittest.TestCase):
    def test_runtime_and_package_version_match(self) -> None:
        ...
        floor = tuple(int(part) for part in setuptools_requirement.removeprefix("setuptools>=").split("."))
        self.assertGreaterEqual(floor, (77, 0, 0))
    ...
    def test_changelog_reflects_current_authored_pack_release(self) -> None:
        ...
        self.assertIn("## [0.2.5] - 2026-07-20", text)
        self.assertIn("## [0.2.4] - 2026-04-15", text)
        self.assertIn("packet mental model", text)
        self.assertLess(text.index("## [0.2.5] - 2026-07-20"), text.index("## [0.2.4] - 2026-04-15"))
    ...
    def test_release_check_has_contributor_and_release_clean_modes(self) -> None:
        ...
        self.assertIn("--release-clean", text)
        self.assertIn("release mode requires a fully clean worktree", text)
        self.assertIn("checks changed tracked files", text)
        self.assertIn('-m build --sdist --wheel', text)
        self.assertIn('-m build --wheel', text)
        self.assertIn("authored-pack --help", text)
        self.assertIn("authored-pack assemble", text)
        self.assertIn("authored-pack inspect", text)
        self.assertIn("authored-pack verify", text)

    def test_sdist_manifest_is_runtime_only(self) -> None:
        manifest = (ROOT / "MANIFEST.in").read_text(encoding="utf-8")
        for excluded in ("tests", "scripts", "docs", ".github", "bin", "assets"):
            self.assertIn(f"prune {excluded}", manifest)
        self.assertIn("recursive-include authored_pack *.py", manifest)
        self.assertIn("global-exclude *.egg-info", manifest)

    def test_gitignore_ignores_local_state(self) -> None:
        text = (ROOT / ".gitignore").read_text(encoding="utf-8")
        self.assertIn(".claude/", text)
        self.assertIn(".control/", text)
        self.assertIn(".local_scratch/", text)
        self.assertIn("docs/goals/", text)
    ...
    def test_public_demo_and_release_assets_exist(self) -> None:
        for rel in (
    ...
    def test_contributing_points_to_release_check(self) -> None:
        ...
        self.assertIn("bash scripts/release_check.sh --release-clean", text)
        for version in ("Python 3.11", "Python 3.12", "Python 3.13"):
            self.assertIn(version, text)

    def test_tui_pty_smoke_completes_all_operator_flows(self) -> None:
        proc = subprocess.run(
            [sys.executable, "scripts/smoke_tui_pty.py"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
        ...
        for case in (
            ...
            self.assertIn(f"ok: {case}", proc.stdout)
    ...
    def test_agents_points_public_surface_work_to_voice_contract(self) -> None:
        ...
        self.assertNotIn("generated_from=", text)
        self.assertNotIn("do_not_edit=true", text)
        self.assertNotIn("mail/account identity", text)
    ...
    def test_public_demo_assets_lead_with_assemble(self) -> None:
        ...
        self.assertIn("The canonical demo verifies the emitted ZIP", demo_doc)
    ...
    def test_release_notes_match_current_public_surface(self) -> None:
        ...
        self.assertIn("Release verification used for `v0.2.5`", text)
        self.assertIn("clean tracked worktree", text)
        self.assertIn("bash scripts/release_check.sh", text)
        self.assertNotIn("Status: public release target", text)
        self.assertNotIn("Before tagging `v0.2.5`, confirm:", text)
        self.assertNotIn("- `stamp`\n- `verify`\n- `stamp-bin`", text)

    def test_private_legacy_and_rights_unclear_material_is_absent(self) -> None:
        for rel in (
            ".control",
            ".local_scratch",
    ...
    def test_tui_help_hides_legacy_insane_alias_but_parser_still_accepts_it(self) -> None:
        ...
        self.assertIn("without a text source, header words stay empty", help_text)
        self.assertNotIn("bundled words", help_proc.stdout)
