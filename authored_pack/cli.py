...
def _ensure_repo_clone_consume_bin_defaults(source_bin: Path, out_dir: Path) -> None:
    ...
    missing_default_source = using_default_source and not DEFAULT_SOURCE_BIN.is_dir()
    missing_default_out = using_default_out and not DEFAULT_AUTHORED_OUT.is_dir()
    if not missing_default_source and not missing_default_out:
        return
    raise _value_error(
        ...
        details={
            ...
            "missing_default_source_bin": bool(missing_default_source),
            "missing_default_out": bool(missing_default_out),
        },
    )
...
def _verification_limits_mib(args: argparse.Namespace) -> Dict[str, int]:
    ...
    if hasattr(args, "max_zip_members"):
        limits["max_zip_members"] = int(args.max_zip_members)
    return limits


def _verification_limits_bytes(args: argparse.Namespace) -> Tuple[int, int, int, int]:
    limits = _verification_limits_mib(args)
    return (
        limits["max_manifest_mib"] * MIB,
        limits["max_artifact_mib"] * MIB,
        limits["max_total_mib"] * MIB,
        int(limits.get("max_zip_members", DEFAULT_MAX_ZIP_MEMBERS)),
    )


def _validated_verification_limits(args: argparse.Namespace) -> Tuple[Dict[str, int], Tuple[int, int, int, int]]:
    limits = _verification_limits_mib(args)
...
def _verify(args: argparse.Namespace) -> int:
    limits, (max_manifest_bytes, max_artifact_bytes, max_total_bytes, max_zip_members) = _validated_verification_limits(args)
...
def _inspect(args: argparse.Namespace) -> int:
    ...
    if int(args.artifact_preview) < 0:
        raise _value_error(
            "--artifact-preview must be >= 0",
            details={"artifact_preview": int(args.artifact_preview)},
        )
    limits, (max_manifest_bytes, max_artifact_bytes, max_total_bytes, max_zip_members) = _validated_verification_limits(args)
    pack_path = _resolve_supported_pack_path(args.pack)
    ...
    if not bool(summary.get("verification_ok")):
        verification_errors = list(summary.get("verification_errors", []))
        message = "verification failed"
        if not args.json and verification_errors:
            message += ": " + "; ".join(str(error) for error in verification_errors)
        raise CliCommandError(
            message,
            error_type="VerificationError",
            details={
                ...
                "pack_root_sha256": summary.get("pack_root_sha256"),
                "payload_root_sha256": summary.get("payload_root_sha256"),
            },
            exit_code=1,
        )
...
def build_parser(*, prog: str = "authored-pack") -> argparse.ArgumentParser:
    ...
    verify.add_argument(
        ...
        help=f"Maximum zip member count to accept while verifying (default: {DEFAULT_MAX_ZIP_MEMBERS})",
    )
    ...
    inspect.add_argument(
        ...
        type=int,
        default=DEFAULT_MAX_ZIP_MEMBERS,
        help=f"Maximum zip member count to accept while inspecting (default: {DEFAULT_MAX_ZIP_MEMBERS})",
    )
    ...
    consume_bin = sub.add_parser(
        "consume-bin",
        aliases=["stamp-bin"],
        help="Advanced: randomly select and move files from a source bin into a new pack",
        description="Randomly select source files, move them into one new pack, and preserve them for recovery if assembly fails.",
    )
