...
Only add typed reports, schemas, modes, or coordination adapters when the task explicitly requires them or the existing runtime contract already depends on them.
For simplification tasks, prefer deletion, inlining, or direct product behavior over new control machinery.
Before adding or accepting any gate, report, scorecard, queue, packet, verdict, schema, wrapper, dashboard, state row, flag, or control surface, identify the live generator or mint path, the live consumer that reads the proposed output, and the exact next action that changes. If any piece is missing, reduce the change instead of adding a surface.
...
- Do not widen scope into attestation, sealed storage, or universal evidence ontology work.
