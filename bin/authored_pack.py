...
def init_theme() -> Theme:
    ...
    if curses.has_colors():
        try:
            ...
            # Keep the calm profile on its established amber identity.
...
class AppState:
    ...
    drop_pending_seen: set[str] = field(default_factory=set)
    ...
    assemble_panel_view_offset: int = 0
...
def _expand_user_path(value: Path | str) -> Path:
    try:
        ...
        raise ValueError(f"path expansion failed for {str(value)!r}: {exc}") from exc


def _selection_view_offset(*, offset: int, selected: int, count: int, capacity: int) -> int:
    if count <= 0 or capacity <= 0:
        return 0
    selected = max(0, min(int(selected), int(count) - 1))
    maximum = max(0, int(count) - int(capacity))
    current = max(0, min(int(offset), maximum))
    if selected < current:
        current = selected
    elif selected >= current + int(capacity):
        current = selected - int(capacity) + 1
...
def _recover_existing_path_suffix(s: str) -> str:
    ...
    for pos in sorted(positions, reverse=True):
        candidate = v[pos:]
        try:
            if _expand_user_path(candidate).exists():
                return candidate
        except (OSError, ValueError):
            continue
    return v
...
def _split_drop_payload(s: str) -> List[str]:
    ...
    if not raw:
        return []
    single = _recover_existing_path_suffix(_clean_dropped_path(raw))
    try:
        if single and _expand_user_path(single).exists():
            return [single]
    except (OSError, ValueError):
        pass
...
def _read_bytes_capped(path: Path, *, max_bytes: int) -> bytes:
    ...
    with path.open("rb") as fh:
        data = fh.read(cap + 1)
    if len(data) > cap:
        raise ValueError(f"text file exceeds {cap} byte import limit")
    return data
...
def _prepare_drop_actions(
    ...
    *,
    ...
    for idx, v in enumerate(paths):
        ...
        try:
            p = _expand_user_path(v)
        except ValueError as exc:
            actions.append(
                DropPreparedAction(
                    message=f"Rejected path: {exc}",
                    terminal=True,
                    seen_key=seen_key,
                )
            )
        ...
        if p.exists() and p.is_file() and p.suffix.lower() in (".txt", ".md", ".markdown"):
            try:
                data = _read_bytes_capped(p, max_bytes=2_000_000)
...
def _start_drop_worker_if_idle(state: AppState) -> None:
    ...
    def _worker() -> None:
        ...
        try:
            ...
            actions = [
                DropPreparedAction(
                    message=f"Drop import failed: {exc}",
                    terminal=True,
                    seen_key=(req.seen_keys[idx] if idx < len(req.seen_keys) else None),
                )
                for idx, _path in enumerate(req.paths)
            ]
...
def _queue_drop_paths(
    ...
    *,
    ...
    for key in keys:
        if key is not None:
            state.drop_pending_seen.add(str(key))
...
def _drain_drop_results(state: AppState) -> None:
    ...
    while True:
        ...
        for act in actions:
            if act.seen_key is not None:
                state.drop_pending_seen.discard(str(act.seen_key))
        changed = True
...
def _dropzone_preview(state: AppState, *, width: int, height: int) -> List[str]:
    ...
    lines.append("   Seven paths are processed per batch; watched-folder extras stay queued.")
    lines.append("")
    ...
    if state.drop_deferred_count:
        lines.append(f"Deferred watched items: {state.drop_deferred_count}")
...
def _sources_dropzone_lines(state: AppState, *, width: int, detailed: bool) -> List[str]:
    ...
    if detailed:
        ...
        line = (
            ...
            f"folder items {state.drop_last_count} | imported {state.drop_import_count} | deferred {state.drop_deferred_count}"
        )
...
def _poll_drop_dir(state: AppState) -> None:
    ...
    for p in items:
        ...
        try:
            ...
            key = str(p)
        if key in state.drop_seen or key in state.drop_pending_seen:
            continue
    ...
    state.drop_deferred_count = max(0, extras)
    if extras > 0:
        state.drop_last_msgs = [f"Deferred drop items: {extras} (processed in later batches)"]
        state.drop_last_msgs_ticks = 40
...
def _load_wordlist_from_text_file(path: Path, *, max_bytes: int = 5_000_000) -> List[str]:
    ...
    """
    try:
        data = _read_bytes_capped(path, max_bytes=int(max_bytes))
    except Exception:
        return []
    try:
...
def _read_text_lines(path: Path, limit: int = 2000, *, max_bytes: int = 2_000_000) -> List[str]:
    try:
        raw = _read_bytes_capped(path, max_bytes=max_bytes).decode("utf-8")
    except Exception as exc:
...
def _show_seed_reveal(state: AppState, seed_master: bytes) -> None:
    ...
    if prior_status in ("Done.", "Failed."):
...
def _effective_verify_path(state: AppState) -> str:
    ...
    if val:
        try:
            return str(_expand_user_path(_normalize_single_path_input(val)))
        except ValueError:
            return val
...
def _assemble_panel_lines(state: AppState, *, width: int, height: int) -> List[str]:
    ...
    footer = [f"Packing from: {'Authored Sources' if cfg.input_mode == 'sources' else 'Folder'}"]
    if cfg.input_mode == "sources":
        footer.append(f"Collected: {len(state.authored_sources)} source{'s' if len(state.authored_sources) != 1 else ''}")
    if cfg.derive_seed and cfg.mix_sources:
        eligible = len(_lockdown_eligible_sources(state.authored_sources))
        footer.append(f"Sources ready for seed: {eligible}/{state.entropy_min_sources}")
    footer.append("Enter on 'Assemble now' writes the pack with these settings.")

    row_capacity = max(2, int(height) - len(header) - min(3, len(footer)))
    regular_rows = rows[:-1]
    regular_capacity = max(1, row_capacity - 1)
    selected_regular = min(int(state.assemble_panel_selected), max(0, len(regular_rows) - 1))
    state.assemble_panel_view_offset = _selection_view_offset(
        offset=state.assemble_panel_view_offset,
        selected=selected_regular,
        count=len(regular_rows),
        capacity=regular_capacity,
    )
    start = state.assemble_panel_view_offset
    shown: List[Tuple[int, AssemblePanelRow]] = list(enumerate(regular_rows[start : start + regular_capacity], start=start))
    shown.append((len(rows) - 1, rows[-1]))

    lines = list(header)
    for idx, row in shown:
    ...
    remaining = max(0, int(height) - len(lines))
    lines.extend(footer[:remaining])
    return [ln[:width] for ln in lines[:height]]
...
def _assemble_preview_lines(state: AppState, *, width: int, height: int) -> List[str]:
    ...
    input_label = "Authored Sources" if cfg.input_mode == "sources" else _display_path(input_path, max_len=44)
    output_label = _display_path(_effective_assemble_output(state, cfg), max_len=44)
    lines = [
...
def _verify_preview_lines(state: AppState) -> List[str]:
    ...
    pack_label = _display_path(pack_path, max_len=44) if pack_path else "not set"
    lines = [
    ...
    *,
    ...
    if auto_selected:
        ...
        if candidate_count > 0:
            lines.append(f"candidate packs found: {candidate_count}")
...
def _authored_sources_preview(state: AppState, *, width: int, height: int) -> List[str]:
    ...
    capacity = max(1, int(height) - len(lines))
    state.entropy_view_offset = _selection_view_offset(
        offset=state.entropy_view_offset,
        selected=sel,
        count=len(state.authored_sources),
        capacity=capacity,
    )
    start = state.entropy_view_offset
    end = min(len(state.authored_sources), start + capacity)
    identity_counts: Dict[str, int] = {}
    for source in state.authored_sources:
        sid = _entropy_source_identity(source)
        identity_counts[sid] = identity_counts.get(sid, 0) + 1
    for i, s in enumerate(state.authored_sources[start:end], start=start):
        mark = ">>" if i == sel else "  "
        meta_bits: List[str] = []
        if s.kind == "photo":
        ...
        sid = _entropy_source_identity(s)
        if identity_counts.get(sid, 0) > 1:
...
def _draw_footer(stdscr, state: AppState, rows: int, cols: int) -> None:
    ...
    if state.viewer is not None:
        legend = "Up/Down/PgUp/PgDn: scroll  Enter/Esc/q/b: back"
...
def _prompt_str_curses(stdscr, label: str, *, default: str = "", max_len: int = 512) -> Optional[str]:
    ...
    try:
        while True:
            ...
            hint = "Type a value. Enter saves. Esc cancels. Ctrl-D clears. Ctrl-A/E move. Ctrl-U/W erase."
            if is_path_like:
                hint = "Type a value. Enter saves. Esc cancels. Single q also cancels path prompts."
            ...
            if isinstance(raw_ch, str):
                if raw_ch in ("\n", "\r"):
                    ...
                    ch = 27
                elif len(raw_ch) == 1 and ord(raw_ch) < 32:
                    ch = ord(raw_ch)
                elif len(raw_ch) == 1 and raw_ch.isprintable():
                    if len(buf) >= effective_max_len:
                        truncated = True
                        continue
                    buf.insert(cursor, raw_ch)
                    cursor += 1
                    continue
                else:
                    continue
...
def _action_entropy_delete_selected(state: AppState) -> None:
    ...
    state.entropy_view_offset = _selection_view_offset(
        offset=state.entropy_view_offset,
        selected=state.entropy_selected,
        count=len(state.authored_sources),
        capacity=1,
    )
    if not state.authored_sources:
...
def _action_entropy_clear(state: AppState, stdscr=None) -> bool:
    n = len(state.authored_sources)
    if n == 0:
        return False
    if stdscr is not None:
        confirmed = _prompt_bool_curses(stdscr, f"(Authored Pack) Clear all {n} staged source(s)", default=False)
        if not confirmed:
            state.status = "Ready."
            state.log_lines = ["Clear cancelled; staged sources were kept."]
            return False
    state.authored_sources.clear()
    state.entropy_selected = 0
    state.entropy_view_offset = 0
    state.focus = "menu"
...
def _open_assemble_panel(state: AppState, selected_key: Optional[str] = None, *, show_advanced: bool = False) -> None:
    ...
    state.assemble_panel_view_offset = 0
...
def _edit_assemble_input(state: AppState, stdscr, cfg: Optional[AssembleConfig] = None, *, allow_sources: bool = True) -> bool:
    ...
    if normalized == "@sources":
        ...
        try:
            _expand_user_path(normalized)
        except ValueError as exc:
            state.status = "Failed."
            state.log_lines = [f"Input path rejected: {exc}"]
            return False
        _set_current_lane(state, "folder")
...
def _edit_assemble_output(state: AppState, stdscr, cfg: Optional[AssembleConfig] = None) -> bool:
    ...
    if raw is None:
        state.status = "Ready."
        return False
    normalized = _normalize_single_path_input(raw) or "./out"
    try:
        _expand_user_path(normalized)
    except ValueError as exc:
        state.status = "Failed."
        state.log_lines = [f"Output path rejected: {exc}"]
        return False
    cfg.out_path = normalized
    state.log_lines = []
...
def _edit_verify_path(state: AppState, stdscr) -> bool:
    ...
    if raw is None:
        state.status = "Ready."
        return False
    normalized = _normalize_single_path_input(raw)
    try:
        _expand_user_path(normalized)
    except ValueError as exc:
        state.status = "Failed."
        state.log_lines = [f"Verify path rejected: {exc}"]
        return False
    state.verify_config.pack_path = normalized
    state.log_lines = []
...
def _run_assemble_plan(
    ...
    *,
    ...
    tmp_source_record_parent: Optional[Path] = None
    source_record_dir: Optional[Path] = None
    source_record_receipt_fields: Dict[str, object] = {}
    ...
    try:
        ...
        if input_choice == "@sources":
            ...
            input_dir = _expand_user_path(input_choice)
            _set_current_lane(state, "folder")
            state.assemble_config.input_mode = "folder"
        ...
        out_dir = _expand_user_path(_normalize_single_path_input(out_s))
        ...
        if write_sources and derive_seed and sources_for_audit:
            tmp_source_record_parent = Path(tempfile.mkdtemp(prefix="authored_pack_source_record_"))
            p, warnings, materialized_count = _write_authored_sources_into_pack(
                tmp_source_record_parent,
                sources_for_audit,
            )
        ...
        def _do_assemble() -> AssembleResult:
            return assemble_pack(
                ...
                print_seed=False,
                source_record_dir=source_record_dir,
                source_record_receipt_fields=source_record_receipt_fields,
            )
        ...
        if tmp_payload_dir is not None:
            try:
                ...
                pass
        if tmp_source_record_parent is not None:
            try:
                shutil.rmtree(tmp_source_record_parent)
            except Exception:
                pass
    ...
    if source_record_dir is not None and (res.pack_dir / "authored_sources").is_dir():
        audit_dir_path = res.pack_dir / "authored_sources"
    if payload_root:
...
def _recent_pack_candidate(parent: Path) -> Tuple[Optional[Path], int]:
    candidates = [path for path in parent.iterdir() if path.is_dir() and (path / "manifest.json").is_file()]
    candidates.sort(key=_candidate_mtime, reverse=True)
...
def _run_verify_plan(state: AppState, stdscr, *, pack_s: str, allow_large_manifest: bool) -> None:
    ...
    state.log_lines = []
    try:
        pack = _expand_user_path(_normalize_single_path_input(pack_s))
    except ValueError as exc:
        state.log_lines = _verify_failure_lines(Path(pack_s), [str(exc)])
        state.status = "Failed."
        return
    auto_selected = False
    candidate_count = 0
    try:
        ...
        if pack.is_dir() and not (pack / "manifest.json").is_file():
            picked, candidate_count = _recent_pack_candidate(pack)
            if picked is not None:
                auto_selected = True
                state.log_lines = [
                    f"Auto-selected recent pack: {_display_path(picked, max_len=44)}",
                    f"Candidate packs found: {candidate_count}",
                ]
                pack = picked
    except Exception as exc:
        state.log_lines = _verify_failure_lines(pack, [f"failed to inspect pack candidates: {exc}"])
        state.status = "Failed."
        return
    try:
        ...
        state.log_lines = _verify_failure_lines(
            pack,
            [str(exc)],
    ...
    if res.ok:
        ...
        if auto_selected:
            ...
            state.log_lines.insert(3, f"candidate_packs_found: {candidate_count}")
...
def handle_key(stdscr, state: AppState, ch: int) -> bool:
    if state.viewer is not None:
        if ch in (10, 13, curses.KEY_ENTER, 27, ord("q"), ord("Q"), curses.KEY_LEFT, curses.KEY_BACKSPACE, 127, 8, ord("b"), ord("B")):
            close_viewer(state)
            return True
        if ch in (curses.KEY_UP, ord("k")):
    ...
    if ch in (ord("q"), ord("Q"), curses.KEY_EXIT):
        draft_changed = state.assemble_panel_draft is not None and state.assemble_panel_draft != state.assemble_config
        if state.authored_sources or draft_changed:
            details: List[str] = []
            if state.authored_sources:
                details.append(f"{len(state.authored_sources)} staged source(s)")
            if draft_changed:
                details.append("an unsaved Assemble draft")
            confirmed = _prompt_bool_curses(stdscr, f"(Authored Pack) Quit and discard {' and '.join(details)}", default=False)
            if not confirmed:
                state.status = "Ready."
                state.log_lines = ["Quit cancelled; staged work was kept."]
                return True
        return False
    ...
    if screen == SCREEN_SOURCES:
        ...
        if ch in (ord("p"), ord("P")):
            ...
            return True
        if ch in (ord("d"), ord("D")) and state.focus == "entropy":
            _action_entropy_delete_selected(state)
            return True
        if ch in (ord("c"), ord("C")) and state.focus == "entropy":
            _action_entropy_clear(state, stdscr)
            return True
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument(
        "--godel-source",
        default=None,
        help="Optional text/markdown path for header words in Noisy Mode. PDFs are ignored; without a text source, header words stay empty.",
    )
